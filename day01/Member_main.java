...
public class Member_main {
	...
	public static void main(String[] args) {
		...
		// Member_main 클래스와 Member 클래스는 동일한 패키지에 저장된 클래스 이므로 import를 안하더라도 사용 가능함. 
	...
	//  lssMbr.address = "서울시 강남구"; 이렇게 사용해도 되지만 static 변수는 아래와 같이 클래스명.변수명으로 사용할 것을 권장한다.
	...
	//  eom_mbr.school = "대학원졸"; -- school 은 Member 클래스에서 존재하지 않는 필드이므로 불가함.
	    ...
	    System.out.println(" >> lssMbr 이라는 인스턴스(객체)에 저장된 필드 값 2번째 << ");
	    lssMbr.print_info();
	    
	    System.out.println(" >> eom_mbr 이라는 인스턴스(객체)에 저장된 필드 값 2번째 << ");
        eom_mbr.print_info();
	}
