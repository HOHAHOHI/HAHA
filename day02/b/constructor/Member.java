...
public class Member {
	...
	Member(){} // 파라미터(== 매개변수)가 있는 생성자와 함께 기본 생성자를 사용할 경우라면 반드시 기본 생성자를 선언해줘야 한다.
	...
	Member(String userid, String passwd, String name, int age, int point){
		/*
		    지역변수(갈색)명과 멤버변수명(instance 변수와 static 변수 모두를 지칭하는 것)이
		    일치할 경우에는 지역변수가 더 우선된다.
		 */
		
		//this는 해당 클래스에 인스턴스를 나타낸다.
		this.userid = userid; 
		this.passwd = passwd;
		this.name = name;
		this.age = age;
    ...
    void update(String userid, String passwd, String name, String email, int age, int point) {
    	...
    	// 생성자 호출(this)은 반드시 생성자 내에서만 가능하다.
